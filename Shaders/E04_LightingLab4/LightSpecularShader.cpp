...
LightSpecularShader::LightSpecularShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	InitShader(L"shaders/lightSpec_vs.hlsl", L"shaders/lightSpec_ps.hlsl");
}


LightSpecularShader::~LightSpecularShader()
{
	if (m_camBuffer)
	{
		m_camBuffer->Release();
...
void LightSpecularShader::InitShader(WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	camBufferDesc.ByteWidth = sizeof(LightBufferType);
...
void LightSpecularShader::Render(ID3D11DeviceContext* deviceContext, int indexCount)
{
